...
//! cargo run --features=debug staging_buffer
//!
...
impl HelloTriangleApplication {
    ...
    pub(crate) fn create_graphics_pipeline(&mut self) {
        let vert_shader_code = Self::read_file(Path::new("src/shader/vertex_input_vert.spv"));
        let frag_shader_code = Self::read_file(Path::new("src/shader/vertex_input_frag.spv"));
...
///
/// 我们现在拥有的顶点缓冲区现在可以正常工作，但是允许我们从CPU访问它的内存类型可能不是图形卡本身读取的最佳内存类型。最佳内存具有该 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT标志，并且通常不能由专用图形卡上的CPU访问
///  
/// 在本章中，我们将创建两个顶点缓冲区。CPU可访问内存中的一个Staging_Buffer，用于将数据从顶点数组上载到设备，最后一个顶点缓冲区在设备本地内存中。然后，我们将使用缓冲区复制命令将数据从Staging_Buffer移动到实际的顶点缓冲区。
///
pub fn main() {
    // 构建顶点数据
    generator_vertices();
