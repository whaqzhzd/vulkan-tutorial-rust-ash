...
//! cargo run --features=debug descriptor_layout_and_buffer
//!
...
use nal::{Matrix4, Vector2, Vector3};
use std::{
...
impl HelloTriangleApplication {
    ...
    fn update_uniform_buffer(&self, current_image: u32) {
        ...
        let model = nal::Matrix::from_axis_angle(&Vector3::z_axis(), 90.0);
        let view = nal::Matrix::look_at_rh(
            &nal::Point3::<f32>::new(2.0f32, 2.0f32, 2.0f32),
            &nal::Point3::new(0.0f32, 0.0f32, 0.0f32),
            &Vector3::z(),
        );
        let proj = nal::Matrix4::<f32>::new_perspective(
            45.0f32,
        ...
        unsafe {
            data.copy_from_nonoverlapping(&ubo, std::mem::size_of_val(&ubo));
    ...
    pub(crate) fn create_graphics_pipeline(&mut self) {
        let vert_shader_code = Self::read_file(Path::new("src/shader/ubo_shader_vert.spv"));
        let frag_shader_code = Self::read_file(Path::new("src/shader/ubo_shader_frag.spv"));
    ...
    pub(crate) fn create_vertex_buffer(&mut self) {
        ...
        let buffer_size = std::mem::size_of_val(&vertices[0]) as u64 * vertices.len() as u64;
    ...
    pub(crate) fn create_index_buffer(&mut self) {
        let buffer_size = std::mem::size_of_val(&INDICES[0]) as u64 * INDICES.len() as u64;
        ...
        let data = unsafe {
            self.device
                ...
                .expect("map_memory error") as *mut u16
        };
